...
prom_crl_last_update = Gauge(
    "ipa_pki_crl_last_update_time_seconds",
    "Timestamp of CRL last update",
    ("path", "issuer"),
    registry=prom_reg,
)
prom_crl_next_update = Gauge(
    "ipa_pki_crl_next_update_time_seconds",
    "Timestamp of CRL next update",
    ("path", "issuer"),
    registry=prom_reg,
)
prom_crl_number = Gauge(
    "ipa_pki_crl_number",
...
prom_status = Gauge(
    "ipa_pki_crl_fetch_completion_time_seconds",
    "Timestamp of successful CRL fetch",
    registry=prom_reg,
)
...
def main(argv):
    ...
    ldap3.utils.log.set_library_log_detail_level(
        int(os.environ.get("IPA_PKI_CRL_FETCH_LDAP_LOG_LEVEL", "0"))
    )
    ...
    server = ldap3.Server(
        os.environ["IPA_PKI_CRL_FETCH_LDAP_URL"],
        connect_timeout=6,
    ...
    while not exit_.is_set():
        ...
        try:
            if not conn:
                ...
                crl_issuing_point_record = ldap3.ObjectDef(
                    ["crlIssuingPointRecord"], conn
                )
            crl_reader = ldap3.Reader(
                conn,
                crl_issuing_point_record,
                base="ou=crlIssuingPoints,ou=ca,o=ipaca",
                sub_tree=False,
                attributes=[
        ...
        for crl in crls:
            try:
                ...
                crl_next_update = datetime.strptime(
                    crl.nextUpdate.value, "%Y%m%d%H%M%SZ"
                ).replace(tzinfo=timezone.utc)
                logger.debug("CRL %r next update at %s", crl.cn.value, crl_next_update)
                earliest_next_update_in = min(
                    earliest_next_update_in, crl_next_update - now
                )

        sleep_for = max(
            timedelta(minutes=1, seconds=random.randint(-15, 14)),
            earliest_next_update_in / 2,
        )
...
def process_crl(crl):
    ...
    try:
        ...
        old_crl_number = old_crl.extensions.get_extension_for_oid(
            x509.CRLNumber.oid
        ).value.crl_number

    new_crl = x509.load_der_x509_crl(crl.certificateRevocationList.value)
    new_crl_number = new_crl.extensions.get_extension_for_oid(
        x509.CRLNumber.oid
    ).value.crl_number

    if old_crl_number != new_crl_number:
        logger.info(
            "CRL %r updated %r → %r", crl.cn.value, old_crl_number, new_crl_number
        )
...
def update_metrics():
    for crl_file in crl_dir.glob("*.crl"):
        ...
        prom_crl_last_update.labels(*labelvalues).set(
            crl.last_update.replace(tzinfo=timezone.utc).timestamp()
        )
        prom_crl_next_update.labels(*labelvalues).set(
            crl.next_update.replace(tzinfo=timezone.utc).timestamp()
        )
        prom_crl_number.labels(*labelvalues).set(
            crl.extensions.get_extension_for_oid(x509.CRLNumber.oid).value.crl_number
        )
